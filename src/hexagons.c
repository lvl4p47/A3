...
int gravcentx = side / 2, gravcenty = side / 2;
...
void EntityCollision(Kvad_t* ptr, Entity_t* p_e)
{
    ...
    else
    {
        ...
        {
            dz += -gx, dn += -gy;
            if(t == gravtime && physics || 1) p_e->fuel -= 2, fuelused = 1;
        }
        if(inpst.vy > 0)
            dz += gx, dn += gy;
        else if(inpst.vx > 0)
...
void KvadGenerate(Kvad_t *ptr, int n)
{
    ...
    else
    {
        for(int i = 0; i < ptr->height; i++)
        {
            for(int j = 0; j < ptr->width; j++)
            {
                if(j + 2 * i > (ptr->width + 2 * ptr->height) / 2)
                    KvadSetMat(ptr, j, i, 8, 1);
            }
        }
